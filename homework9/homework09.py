...
def deco_csv(function: Callable):
    ...
    def wrapper():
        with open('random_int_file.csv', 'r', encoding='UTF-8') as file:
...
def json_result(func: Callable):
    result = {}
    if os.path.exists('solving_quadratic_equations.json'):
        with open('solving_quadratic_equations.json', 'r', encoding='UTF-8') as file:
            result = json.load(file)
    else:
        with open('solving_quadratic_equations.json', 'w', encoding='UTF-8') as file:
...
def create_coefficients_csvfile():
    with open('random_int_file.csv', 'w', newline='', encoding='UTF-8') as file:
        writer = csv.writer(file, quoting=csv.QUOTE_NONNUMERIC)
        for row in range(rnd(100, 1000)):
